...
function Gameplay() {
    ...
    if (
      ...
      return (
        ...
        <button
          ...
          id="specialButton"
          onClick={() =>
            handleSpecialButtonClick(option.option_id, characterClass)
          }
        >
          {option.option_name}
        </button>
      ...
      return (
       ...
       return (
        ...
        <button
          ...
          key={option.option_id}
          onClick={() =>
            handleButtonClick(option.destination_situation, option.option_text)
          }
        >
          {option.option_name}
        </button>
  ...
  const handleSpecialButtonClick = (optionId, characterClass) => {
    ...
    if (specialButton && !specialButton.classList.contains("disabled")) {
      // make it check state for specialbuttons array objects with special buttons option, and enabled
  ...
  const saveGameHandler = async (currentSituation, userId) => {
    try {
      const res = await fetch(
        `http://localhost:5431/update-situation/${userId}`,
        {
          method: "POST",
          headers: {
  ...
  const loadOptions = async (currentSituation) => {
    try {
      const res = await fetch(
        `http://localhost:5431/options/${currentSituation}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:5431/situation/${currentSituation}`
        );
        const situationData = await res.json();
        setImage(situationData[0].image_link);
        ...
        setSkillCheckOptions((prevOptions) =>
          prevOptions.map((option) =>
            option.optionId === 5
              ? { ...option, enabled: specialButtonEnabled }
              : option
          )
        );
      } catch (error) {
